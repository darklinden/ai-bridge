#!/usr/bin/env bash
# 本地 OpenAI Chat Completions 入口测试（POST /v1/chat/completions）。
# 默认流式（SSE），用于验证 [REQ]/[RESP] 日志与响应转换。
#
# 用法:
#   ./test-chat.sh [--no-stream] [提示词]
#
# 说明:
...
echo "==> POST $BASE_URL/v1/chat/completions  (stream=$STREAM)"
echo "==> prompt: $PROMPT"
curl -N -sS --max-time 120 -X POST "$BASE_URL/v1/chat/completions" \
  "${AUTH_HEADER[@]}" \
  -H 'Content-Type: application/json' \
  -d "{\"model\":\"anything\",\"stream\":$STREAM,\"messages\":[{\"role\":\"system\",\"content\":\"你是一个乐于助人的助手。\"},{\"role\":\"user\",\"content\":\"$PROMPT\"}]}"
echo
echo "==> done"