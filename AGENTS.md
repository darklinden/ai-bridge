...
- **Cross-cutting mechanisms** — shared concerns centralized because both channels need them:
  ...
  - `vision.rs` — when a text-only upstream receives images and a `VISION_*` config exists, describes the images via the vision model (multi-image joint call, non-streaming, in-process fingerprint TTL cache). Vision failure degrades to `[Unsupported Image]` and does not block the request.
