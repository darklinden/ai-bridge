...
//!
//! Ported from `src-tauri/src/proxy/tool_media.rs`.
//!
//! Responses and Anthropic tool outputs may carry structured media blocks.
//! Chat Completions tool messages are text-only, so the bridge extracts those
//! blocks and re-emits them in a synthetic user message.
...
mod tests {
    ...
    fn loose_data_image_url_is_media_but_loose_remote_url_is_not() {
        ...
        assert!(tool_output_contains_media(&data, ToolMediaScope::ImagesOnly));
        assert!(!tool_output_contains_media(&remote, ToolMediaScope::ImagesOnly));
    }
    ...
    fn does_not_scan_embedded_data_urls_inside_plain_text() {
        ...
        assert!(!tool_output_contains_media(&value, ToolMediaScope::AllSupported));
        assert_eq!(
            strip_media_direct(&mut value, &mut media, ToolMediaScope::AllSupported, &replacement, "moved"),
            0
        );
    ...
    fn strips_media_from_json_string_and_nested_content() {
        ...
        let replaced = strip_media_direct(
            &mut value,
            &mut media,
            ToolMediaScope::AllSupported,
            &replacement,
    ...
    fn image_only_scope_ignores_file_and_audio() {
        ...
        assert!(!tool_output_contains_media(&file, ToolMediaScope::ImagesOnly));
        assert!(!tool_output_contains_media(&audio, ToolMediaScope::ImagesOnly));
        assert!(tool_output_contains_media(&file, ToolMediaScope::AllSupported));
        assert!(tool_output_contains_media(&audio, ToolMediaScope::AllSupported));
    }
    ...
    fn clamp_preserves_long_text_but_removes_data_and_base64_payloads() {
        ...
        let data_url = large_image_data_url();
        let base64 = "A".repeat(18_000); // looks like base64, long enough
        let mut value = json!({
            "text": long_text,
            "data_url": data_url,
            "raw": base64
    ...
    // ------------------------------------------------------------------
    // Test helpers: expose strip_media_from_tool_value without clamping
    // for tests that want the base (no-clamp) behavior.
    // ------------------------------------------------------------------
    fn strip_media_direct(
        value: &mut Value,
        media_parts: &mut Vec<Value>,
        scope: ToolMediaScope,
        replacement_block: &Value,
    ...
    /// Read-only media detection using the same shape classifier.
    /// (kept as a test helper; not exported from the production crate)
    fn tool_output_contains_media(value: &Value, scope: ToolMediaScope) -> bool {
